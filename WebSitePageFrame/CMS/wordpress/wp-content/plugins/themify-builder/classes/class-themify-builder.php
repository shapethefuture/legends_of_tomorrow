...
?>
	...
	function load_admin_interface($hook) {
		...
		if (in_array($hook, array('post-new.php', 'post.php')) && in_array(get_post_type(), themify_post_types()) && Themify_Builder_Access_Role::check_access_backend()) {
	...
	function is_fullwidth_layout_supported() {
        ...
        public static function getMapKey(){
            static $key = false;
            if($key===false){
                $key = themify_builder_get( 'setting-google_map_key' );
            }
            return $key;
        }
	...
	function get_pagenav( $before = '', $after = '', $query = false, $original_offset = 0 ) {
		...
		$max_page = $numposts / $query->query_vars['posts_per_page'];
		$out = '';
