			...
			$('.style_apply_all').each(function() {
				var $val = $(this).val(),
					$fields = $(this).closest('.themify_builder_field').prevUntil('h4'),
                                        $last = $fields.last(),
					$inputs = $last.find('input[type="text"]'),
					...
					$callback = function() {
						if ($fields.first().next('.themify_builder_field').find('.style_apply_all').is(':checked')) {
							...
							$fields.not(':last').each(function(){
								if ($select) {
                                                                        $(this).find('select option').prop('selected', false).filter('[value="' + $val + '"]').prop('selected', true).trigger('change');
								} else {
		...
		renderColumn: function( $column, sendData ) {
			...
			return $.ajax({
				...
				success: function( data ) {
					...
					$('.current_selected_column').get(0).innerHTML = $newColumn.get(0).innerHTML;
		...
		copyComponentBuilder: function(event) {
			...
			switch (component) {
				...
				case 'sub-column':
					...
					console.log( component, rowData, columnOrder, columnDataInJson, 'columnOrder' );
