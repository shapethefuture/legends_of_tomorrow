	...
	function themify_theme_mega_get_posts( $term_id, $taxonomy ){
		...
		if( $posts ) {
			...
			foreach( $posts as $post ) {
				setup_postdata( $post );
				get_template_part( 'includes/loop-megamenu', get_post_type() );
			}
...
function themify_menu_mega_option( $item_id, $item, $depth, $args ) {
	...
	if ( 'taxonomy' == $item->type || 'custom' == $item->type || ( 'post_type' == $item->type && 'page' == $item->object ) ) {
		...
		?>
		<div class="tf-mega-option">
			<p class="field-tf-mega description description-thin">
				<label for="edit-menu-item-tf-mega-<?php echo esc_attr( $item_id ); ?>">
					<input type="checkbox" name="menu-item-tf-mega[<?php echo esc_attr( $item_id ); ?>]" id="edit-menu-item-tf-mega-<?php echo esc_attr( $item_id ) ?>" class="edit-menu-item-tf-mega themify_field_tf-mega" value="1" <?php checked( $is_mega, true ) ?>> <?php _e( 'Enable Mega Posts', 'themify' ) ?>
				</label>
			</p>
			<p class="field-tf-mega description description-thin">
				<label for="edit-menu-item-tf-mega_column-<?php echo esc_attr( $item_id ); ?>">
					<input type="checkbox" name="menu-item-tf-mega_column[<?php echo esc_attr( $item_id ); ?>]" id="edit-menu-item-tf-mega_column-<?php echo esc_attr( $item_id ) ?>" class="edit-menu-item-tf-mega_column themify_field_tf-mega" value="1" <?php checked( $is_column, true ) ?>> <?php _e( 'Enable columns menu', 'themify' ) ?>
				</label>
			</p>
		</div>
		<script type="text/javascript">
			(function($){
				/**
				 * Show dropdown to choose a taxonomy when behaviour is set to Mega.
				 * @param { object } $checkbox Select object with behaviour options.
				 */
				function toggleMegaOption( $checkbox ) {
					var $wrapper = $checkbox.closest( '.tf-mega-option' );
					if ( $checkbox.hasClass( 'edit-menu-item-tf-mega' ) ) {
						$wrapper.find( '.edit-menu-item-tf-mega_column' ).prop( 'checked', false );
					} else if ( $checkbox.hasClass( 'edit-menu-item-tf-mega_column' ) ) {
						$wrapper.find( '.edit-menu-item-tf-mega' ).prop( 'checked', false );
					}
				}
				// On select change.
				$( '#nav-menus-frame' ).on( 'click', '.themify_field_tf-mega', function( e ) {
					toggleMegaOption( $( e.currentTarget ) )
				} );
			})(jQuery);
		</script>
		<?php
...
function themify_update_mega_menu_option( $menu_id, $menu_item_db_id, $args ) {
	$meta_keys = array(
		'_themify_mega_menu_item'   => 'menu-item-tf-mega',
		'_themify_mega_menu_column' => 'menu-item-tf-mega_column',
		'_themify_mega_menu_column_sub_item' => 'menu-item-tf-mega_column_sub_item',
...
class Themify_Widgets_Menu {
	...
	function wp_nav_menu_item_custom_fields( $item_id, $item, $depth, $args ) {
		if( $this->is_menu_widget( $item ) ) : ?>
			...
			<div class="themify-widget-options" style="clear: both;">
				<?php echo $this->get_widget_form( $this->get_widget_menu_class( $item ), $item_id, $item ); ?>
			</div>
		<?php endif;
	}
	...
	function nav_menu_script() {
		...
		// wp_enqueue_style( 'themify-megamenu-admin', THEMIFY_MEGAMENU_URI . '/assets/admin.css' );
